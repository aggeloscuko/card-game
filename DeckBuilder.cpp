...
DeckBuilder::~DeckBuilder()
{
	...
	for (it = black->begin(); it != black->end(); it++)	//maybe one more delete after the for loop
	{
		black->remove((*it));
		delete *it;
		it = black->begin();				//it++? What the f*ck?
	}
...
list<BlackCard *>* DeckBuilder::getBlack(void) const
{
	return black;
}

list<GreenCard *>* DeckBuilder::getGreen(void) const
{
	return green;
}
